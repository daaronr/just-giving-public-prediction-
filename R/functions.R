...
doTest_noatr <- function(pair, df = ADSX, stage = 2, depvar = donation, treatvar = Treatment, testname = "t.test2") {
  require("dplyr")
  depvar <- enquo(depvar)
  treatvar <- enquo(treatvar)
  thetest <- match.fun(testname)
...
doFisher <- function(pair,df=ADSX,stage=2,depvar=d_donation,treatvar=Treatment){
  ...
  treatvar <- enquo(treatvar)
  ADSplit <- df %>% ungroup() %>%
    filter(!is.na(depvar) & Stage == stage) %>%
    split(pull(., !!treatvar)) %>%
    map(~ dplyr::select(., depvar)) %>%
    extract(pair)
...
# ...(will take nonmissing value if only one present, takes value from 'x' if otherwise conflicting)
# arguments: y and x are table names, by is shared key(s?) (in quotes)
merge_cols <- function(x, y, by) {
  ...
  # y_select <- dplyr::select(y, shared_columns)
  # We don't want NAs in the by columns of y, to avoid ballooning NA joins.
  y_select_filter <- dplyr::select(y, by)
  y_select_filter <- y_select_filter %>%
    dplyr::filter_all(dplyr::all_vars(!is.na(.)))
  # y_select <- y_select %>%
  y <- y %>%
    dplyr::inner_join(y_select_filter)

  # JohnH: Join x to the shared columns in y.
  # joined_tibbles <- dplyr::left_join(x, y_select, by) %>% dplyr::distinct()
  # US: Join x to ALL columns in y.
  joined_tibbles <- dplyr::left_join(x, y, by) %>% dplyr::distinct() # DR: "distinct" is superfluous in our case

  # For each duplicated column, find the "best" results (replace NAs in x with values in y).
  # US: ... AND vice/versa (if possible)
  best_columns <- purrr::map_dfc(duplicated_columns, function(column_name) {
    #-- applies this function to each duplicated column
    internal_return <- dplyr::tibble(
      col1 = dplyr::coalesce(joined_tibbles[[paste0(column_name, ".x")]], joined_tibbles[[paste0(column_name, ".y")]])
    )
  ...
  # Bind the tibbles back together, and get rid of the interim columns.
  dplyr::bind_cols(joined_tibbles, best_columns) %>%
    # dplyr::select(colnames(x)) %>% JohnH selects x columns only; I think we want both
    dplyr::select(union(colnames(x), colnames(y))) %>%
    dplyr::distinct()
}

# summary tables function(s)

sumtab_func_full <- function(df = ADSX, depvar = donation, treatvar = TreatFirstAsk, caption="") {
  treatvar <- enquo(treatvar)
  depvar <- enquo(depvar)
  df %>%
    ungroup() %>%
...
sumtab_func <- function(df = ADSX, depvar = donation, treatvar = TreatFirstAsk, caption="") {
  treatvar <- enquo(treatvar)
  depvar <- enquo(depvar)
  df %>%
    ungroup() %>%
...
#  dplyr::summarize(N = n(),
#                   Mean = round(mean(donation, na.rm = T), 2),
#                   "Std.dev." = glue("(", {
#                     round(sd(donation, na.rm = T), 2)
#                   }, ")")

#  ) %>%
# unite(Results, Mean, "Std.dev.", N, sep = " ") %>%
# spread(Stage, Results) %>% rename("Stage 1" = `1`, "Stage 2" = `2` )%>%
#  separate("Stage 1",  c("Mean", "Std.dev.", "N"), sep = " ") %>%
...
#Just a shortcut to adding the usual adorns to save typing
adornme <-  function(atabyl, adorn = "row",digits=2,cap="", title="") {
  atabyl %>%
    adorn_totals(c("row", "col")) %>%
  adorn_percentages(adorn) %>%
  adorn_pct_formatting(digits = 2) %>%
  adorn_ns() %>%
    adorn_title(title, placement = "top") %>%
    kable(caption=cap) %>%
  kable_styling()
}
