...
metadata(u_fdd_fd)$name <- "Justgiving fundraising pages, aggregated donations"
metadata(u_fdd_fd)$description <- "Dataset: JustGiving pages pulled via API 2017-19 for 'highly effective' charities; reduced to one row per fundraiser, donation data merged back in; event date more than 25 weeks ago or expiry date passed"
metadata(u_fdd_fd)$identifier <- "doi:TBA"
metadata(u_fdd_fd)$datePublished <- "TBA"
metadata(u_fdd_fd)$creator <- list(
  list(
  "@type" = "Person",
...
metadata(u_fdd_fd)$citation <- "Jolly, Reinstein, Riener (2019)"
metadata(u_fdd_fd)$url <-
  "TBA"
metadata(u_fdd_fd)$temporalCoverage <- "2007-2019 (mainly 2017-19)"
metadata(u_fdd_fd)$spatialCoverage <- "Mainly UK"
metadata(u_fdd_fd)$keywords <- c("Charitable giving","Social fundraising")
...
codebook(u_fdd_fd, survey_repetition = "single", metadata_table = TRUE)
